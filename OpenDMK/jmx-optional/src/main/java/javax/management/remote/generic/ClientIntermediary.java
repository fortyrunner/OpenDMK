...
//----------------------------------------------
  ...
  private class GenericClientCommunicatorAdmin
    ...
    public void reconnectNotificationListeners(ClientListenerInfo[] old)
      ...
      //
      for (ClientListenerInfo clientListenerInfo : old) {
        try {
          Integer id = addListenerWithSubject(
            clientListenerInfo.getObjectName(),
            serialization.wrap(clientListenerInfo.getNotificationFilter()),
            clientListenerInfo.getDelegationSubject(),
            false);

          clis[j++] = new ClientListenerInfo(id,
            clientListenerInfo.getObjectName(),
            clientListenerInfo.getListener(),
            clientListenerInfo.getNotificationFilter(),
            old[j].getHandback(),
            clientListenerInfo.getDelegationSubject());
        } catch (InstanceNotFoundException infe) {
          logger.warning("reconnectNotificationListeners",
            "Can't reconnect a listener for " +
              clientListenerInfo.getObjectName(), infe);
        }
      }
