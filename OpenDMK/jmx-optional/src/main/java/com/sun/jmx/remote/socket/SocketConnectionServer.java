...
public class SocketConnectionServer implements MessageConnectionServer {
  ...
  public SocketConnectionServer(JMXServiceURL addr, Map env)
    ...
    wildcard =
      (wildcardS == null) || wildcardS.equalsIgnoreCase("true");
...
// implements MessageConnectionServer interface
  ...
  public void start(Map env) throws IOException {
    ...
    try {
      ...
      if (wildcard) {
        Constructor ct = c.getDeclaredConstructor(int.class);
    ...
    if (o != null && DefaultConfig.getServerReuseAddress(newEnv)) {
      try {
        ...
        Method m1 = cc.getMethod("setReuseAddress", boolean.class);

        Method m2 = cc.getMethod("bind", Class.forName("java.net.SocketAddress"), int.class);


        ss = (ServerSocket) cc.newInstance();

        // setReusAddress
        m1.invoke(ss, Boolean.TRUE);

        // bind
        m2.invoke(ss, o, new Integer(DEFAULT_BACKLOG));
