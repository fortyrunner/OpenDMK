...
public abstract class ClientNotifForwarder {
  ...
  public synchronized void postReconnection(ClientListenerInfo[] listenerInfos)
    ...
    for (ClientListenerInfo listenerInfo : listenerInfos) {
      if (trace) {
        logger.trace("addNotificationListeners",
          "Add a listener at " +
            listenerInfo.getListenerID());
      }

      infoList.put(listenerInfo.getListenerID(), listenerInfo);
    }
    ...
    public void run() {
      ...
      while (!shouldStop()) {
        ...
        synchronized (ClientNotifForwarder.this) {
          ...
          for (final TargetedNotification tn : notifs) {
