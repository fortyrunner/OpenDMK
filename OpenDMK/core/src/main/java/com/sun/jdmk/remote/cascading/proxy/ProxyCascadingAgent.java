...
public class ProxyCascadingAgent extends CascadingAgent {
   ...
   *                         <p>
   * @throws IllegalArgumentException if <var>targetPath</var> is
   ...
   * @throws IOException           if the connection with the source
   *                               <tt>MBeanServer</tt> fails.
   * @throws IllegalStateException if this cascading agent is not
  ...
  public synchronized void start(boolean conflictAllowed)
    ...
    try {
      ...
      for (Object mbean : mbeans) {
        final ObjectName sourceName = (ObjectName) mbean;
    ...
    if (failure != null) {
      try {
        ...
        cleanup(false);
        throw failure;
      } catch (IOException | Error | InstanceAlreadyExistsException x) {
        throw x;
      } catch (Throwable x) {
        final IOException io = new IOException("Failed to start: " + x);
        Utils.initCause(io, x);
   ...
   *
   * @throws IOException           not thrown in this default implementation.
  ...
  public Set getCascadedMBeans() {
    ...
    try {
      ...
      for (Object o : sprutstc) {
        final ObjectInstance moi = (ObjectInstance) o;
   ...
   *
   * @throws IOException if the connection with the cascaded
   *                     <tt>MBeanServer</tt> fails.
   **/
  public synchronized void update() throws IOException {
    ...
    for (ObjectName name : names) {
      final ObjectName targetName = getTargetName(name);
      if (sprutstc.remove(name)) {
        // Name found in cascaded MBS. show it.
        showMBean("update", name);
      } else {
        // Name not found! hide it.
        hideMBean("update", name);
      }
    }
    for (Object o : sprutstc) {
      // show remaining mbeans.
      showMBean("update", (ObjectName) o);
    }
    if (logger.finerOn()) {
   ...
   *
   * @throws IllegalStateException if the <tt>ProxyCascadingAgent</tt>
   *                               is not stopped.
    ...
    try {
      synchronized (this) {
        ...
        if (JMXConnectionNotification.OPENED.equals(nt) ||
          ...
          if (JMXConnectionNotification.CLOSED.equals(nt)) {
            stopIfClosed();
          } else {
            if (JMXConnectionNotification.FAILED.equals(nt)) {
              stop(true);
            }
          }
        }
  ...
  private synchronized void stop(boolean connectionDown)
    ...
    if (state.equals(State.STOPPED)) {
      if (logger.fineOn()) {
        logger.fine("stop", "Already " + state);
      }
      return;
    }
    if (!state.equals(State.STARTED)) {
        ...
        try {
          ...
          if (logger.finestOn()) {
            logger.finest("clearProxies",
              "Unregistered target proxy: " + name);
          }
        } catch (Exception x) {
