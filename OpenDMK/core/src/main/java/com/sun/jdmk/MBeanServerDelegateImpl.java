...
final class MBeanServerDelegateImpl
  ...
  final public ObjectName preRegister(MBeanServer server, ObjectName name)
    ...
    if (name == null) {
      return new ObjectName(ServiceName.DELEGATE);
    } else {
      return name;
    }
  }
  ...
  public Object getAttribute(String attribute)
    ...
    } catch (AttributeNotFoundException | JMRuntimeException x) {
      throw x;
    } catch (Exception x) {
  ...
  public AttributeList getAttributes(String[] attributes) {
    ...
    for (String s : attn) {
      try {
        final Attribute a =
          new Attribute(s, getAttribute(s));
        list.add(a);
      } catch (Exception x) {
        // Skip the attribute that couldn't be obtained.
        //
        if (dbg) {
          logger.finer("getAttributes", "Attribute " + s +
            " not found: " + x);
