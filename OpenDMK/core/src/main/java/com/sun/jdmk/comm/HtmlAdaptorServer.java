...
public class HtmlAdaptorServer
  ...
  public HtmlAdaptorServer(int port, AuthInfo[] authInfoList) {
    ...
    if (authInfoList != null) {
      for (AuthInfo info : authInfoList) {
        addUserAuthenticationInfo(info);
      }
    }
  }
   ...
   * @throws InstanceNotFoundException The HTML parser does not exist in the repository.
   * @throws ServiceNotFoundException  The requested service is not supported.
   */
   ...
   *                   parser
   * @throws MalformedObjectNameException   The string passed in the
   *                                        parameters does not have the right format.
   * @throws ReflectionException            Wraps a {@link ClassNotFoundException}
   *                                        or a {@link java.lang.Exception} that occurred when trying
   *                                        to invoke the MBean's constructor.
   * @throws InstanceAlreadyExistsException The MBean is already under
   ...
   * @throws AttributeNotFoundException The specified attribute does not exist or cannot be retrieved.
   ...
   * @throws AttributeNotFoundException     The specified attribute does not exist or cannot be retrieved.
   * @throws InvalidAttributeValueException The specified value is not a valid value for the attribute.
  ...
  private void buildInfo() {
    ...
    for (Constructor constructor : ctor) {
      if (constructor.getParameterTypes().length == 0) {
        dconstructors[0] = new MBeanConstructorInfo("Instantiate HtmlAdaptorServer with default port number equal to " + com.sun.jdmk.ServiceName.HTML_ADAPTOR_PORT + ".", constructor);
      } else {
        if (constructor.getParameterTypes().length == 1) {
          dconstructors[1] = new MBeanConstructorInfo("Instantiate HtmlAdaptorServer with the specified port number.", constructor);
        } else {
          dconstructors[2] = new MBeanConstructorInfo("Instantiate HtmlAdaptorServer with the specified port number and user authentication information list.", constructor);
        }
      }
    }
   ...
   */
  private final String ddescription = "HtmlAdaptorServer class: Provides a management interface of an agent to Web browser clients.";

  /**
   * @serial The list of exposed attributes for the HTML protocol adaptor.
   */
  private final MBeanAttributeInfo[] dattributes = new MBeanAttributeInfo[12];

  /**
   * @serial The list of public constructors for the HTML protocol adaptor.
   */
  private final MBeanConstructorInfo[] dconstructors = new MBeanConstructorInfo[3];

  /**
   * @serial The list of operations for the HTML protocol adaptor.
   */
  private final MBeanOperationInfo[] doperations = new MBeanOperationInfo[5];
   ...
   */
  private final Vector authInfo = new Vector();
