...
public class Introspector {
  ...
  private static Class findMBeanInterface(Class aClass, String aName) {
    ...
    while (current != null) {
      ...
      for (Class anInterface : interfaces) {
        final Class inter =
          implementsMBean(anInterface, aName);
        if (inter != null) {
          return inter;
        }
      }
   ...
   * @throws NotCompliantMBeanException The tested class is not a
   *                                    JMX compliant MBean
   */
      ...
      try {
        ...
        if (name.startsWith("get") && !name.equals("get")
          ...
          if (name.startsWith("set") && !name.equals("set")
            ...
            if (name.startsWith("is") && !name.equals("is")
              && argCount == 0
  ...
  private static boolean testConsistency(List/*<MBeanAttributeInfo>*/attributes,
    ...
    for (Object attribute : attributes) {
      MBeanAttributeInfo mb = (MBeanAttributeInfo) attribute;
  ...
  static Class implementsMBean(Class c, String clName) {
    ...
    for (Class anInterface : interfaces) {
      ...
      try {
        if (anInterface.getName().compareTo(clName + "MBean") == 0) {
          return anInterface;
        }
      } catch (Exception e) {
        return null;
