...
class HtmlArrayPage
  ...
  public void buildPage(String req) {
    ...
    String objNameStr = req.substring(index);
    ...
    if (o != null) {
      ...
      // READ ONLY
        ...
        if (attinfo[k].isReadable() && !attinfo[k].isWritable()) {
          if (logger.finerOn()) {
          ...
          if (attinfo[k].isReadable() && attinfo[k].isWritable()) {
            ...
            for (int i = 0; i < ob.length; i++) {
              ...
              if (typeStr.endsWith("Boolean") || typeStr.endsWith("boolean")) {
                ...
                //
                add2Page("<TD>");
                add2Page(boolToHtml(prop + "[" + i + "]", typeStr, (ob[i] != null ? ob[i].toString() : "null"), (ob[i] != null)));
                add2Page("</TD>");
              } else {
                if (typeStr.equals("javax.management.ObjectName")) {
  ...
  private String buildEleTable(Object obj) {
    ...
    for (int i = 0; i < methods.length; i++) {
      ...
      if (mName.startsWith("get")) {
        ...
        propertyName[i] = mName.substring(3);
        propertyType[i] = usualType(returnType.getName());
    ...
    // Build the HTML Page
    //
    StringBuilder htmlProperty = new StringBuilder();
