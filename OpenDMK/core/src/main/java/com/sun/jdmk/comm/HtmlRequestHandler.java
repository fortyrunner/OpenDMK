...
class HtmlRequestHandler extends ClientHandler {
  ...
  protected boolean authenticateRequest(HttpRequest request) throws IOException {
    ...
    final String authorization =
      request.getHeader(HttpMessage.AUTHORIZATION_HEADER);
  ...
  protected HttpResponse processGetRequest(HttpRequest request) throws IOException {
    ...
    while (true) { // Should make this loop only once.
      ...
      // Handle the request to view a specific m-bean.
        ...
        if (URIPath.startsWith(HtmlDef.VIEWOBJECTRES)) {
        ...
        // Handle the auto refresh request.
          ...
          if (URIPath.startsWith(HtmlDef.AUTOREFRESH)) {
          ...
          // Handle the request to view a property (for array).
            ...
            if (URIPath.startsWith(HtmlDef.VIEWPROPERTY)) {
            ...
            // Set (when apply)
              ...
              if (URIPath.startsWith(HtmlDef.SETFORM)) {
                int query;
                int index;
                String setReqStr;
              ...
              // Build the admin page.
                ...
                if (URIPath.startsWith(HtmlDef.ADMIN)) {
                  String objNameStr;
                ...
                // Build perform action page.
                  ...
                  if (URIPath.startsWith(HtmlDef.INVOKEACTION)) {
                    String objNameStr;
  ...
  protected HttpResponse makeErrorResponse(int statusCode, String msg) {
    ...
    response.setHeader(HttpMessage.CONTENT_TYPE_HEADER, "text/html");
    response.setHeader(HttpMessage.DATE_HEADER, new Date().toString());
    final String wwwauth =
      (statusCode == HttpResponse.STATUS_UNAUTHORIZED) ?
        "Basic realm=\"JDMK\"" : null;
    response.setHeader(HttpMessage.WWW_AUTHENTICATE_HEADER, wwwauth);
