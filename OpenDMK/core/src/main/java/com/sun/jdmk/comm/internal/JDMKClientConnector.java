...
//-------------------------------------
  private class MBeanServerConnectionImpl implements MBeanServerConnection {
    ...
    public String[] getDomains() throws IOException {
      ...
      for (Object n : ns) {
        String d = ((ObjectName) n).getDomain();
  ...
  private class HeartBeatNotifListener implements NotificationListener {
    ...
    public void handleNotification(Notification notif, Object hd) {
      ...
      if (HeartBeatNotification.CONNECTION_ESTABLISHED.equals(hbn.getType())) {
        ...
        if (HeartBeatNotification.CONNECTION_LOST.equals(hbn.getType())) {
          final JMXConnectionNotification jn = new JMXConnectionNotification(
          ...
          if (HeartBeatNotification.CONNECTION_REESTABLISHED.equals(hbn.getType())) {
            final JMXConnectionNotification jn = new JMXConnectionNotification(
              JMXConnectionNotification.OPENED,
              this,
              connectionId,
            ...
            if (HeartBeatNotification.CONNECTION_RETRYING.equals(hbn.getType())) {
              // do nothing
            } else {
              if (HeartBeatNotification.CONNECTION_TERMINATED.equals(hbn.getType())) {
                final JMXConnectionNotification jn = new JMXConnectionNotification(
