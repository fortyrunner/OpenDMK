...
class ServerNotificationDispatcher {
  ...
  protected Object[] clearNotifications(Object[] params) {
    ...
    ClientInfo ci = (ClientInfo) clientList.get(params[0]);
    ...
    synchronized (ci) {
      ...
      if (ci.forwardMode != p) {
        if (p == ClientNotificationHandler.PUSH_MODE) {
          ...
          if (p == ClientNotificationHandler.PULL_MODE) {
  ...
  protected Object[] setOverflowMode(Object[] params) {
    ...
    if (logger.finerOn()) {
      logger.finer("setOverflowMode", "Set the overflow mode to " + params[1] + " for the client " + params[0]);
    }
  ...
  protected Object[] getOverflowMode(Object[] params) {
    ...
    if (logger.finerOn()) {
      logger.finer("getOverflowMode", "Get the overflow mode of the client " + params[0]);
    }
  ...
  protected Object[] setCacheSize(Object[] params) {
    ...
    if (logger.finerOn()) {
      logger.finer("setCacheSize", "Set the cache size of the client " + params[0]);
    }
      ...
      if (ci.notifLog.size() > newSize) {
        ...
        if (((Boolean) params[2]).booleanValue()) {
          ...
          if (ci.discard_mode == ClientNotificationHandler.DISCARD_OLD) {
            ...
            if (ci.discard_mode == ClientNotificationHandler.DISCARD_NEW) {
  ...
  protected Object[] getCacheSize(Object[] params) {
    ...
    if (logger.finerOn()) {
      logger.finer("getCacheSize", "Get the cache size of the client " + params[0]);
    }
  ...
  protected Object[] setOverflowCount(Object[] params) {
    ...
    if (logger.finerOn()) {
      logger.finer("setOverflowCount", "Set the overflow count of the client " + params[0]);
    }
  ...
  protected Object[] getOverflowCount(Object[] params) {
    ...
    if (logger.finerOn()) {
      logger.finer("getOverflowCount", "Get the overflow count of the client " + params[0]);
    }
  ...
  private class ClientInfo {
    ...
    public final List notifLog = Collections.synchronizedList(new ArrayList());
    ...
    public void terminate() {
      ...
      synchronized (rliList) {
        for (Object o : rliList.values()) {
          RemoteListenerInfo rli = (RemoteListenerInfo) o;
          try {
            mbServer.removeNotificationListener(rli.mbean, rli);
          } catch (Exception e) {
            if (ciLogger.finestOn()) {
  ...
  private class PushNotification {
    private final List waitingClientList = Collections.synchronizedList(new ArrayList());
    private TaskThread taskThread = null;

    private final boolean tobeTerminated = false;
  ...
  private final HashMap clientList = new HashMap();
