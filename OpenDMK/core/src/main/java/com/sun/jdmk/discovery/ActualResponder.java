...
class ActualResponder extends DiscoveryCommon {
  ...
  public void run() {
    ...
    if (!spy) {
      if (sendEvent) {
        sendEvent(DiscoveryResponder.ONLINE);
      }
    }
    // ------------------------
    // Infinite loop
    ...
    try {
      while (!stopRequested) {
        try {
          ...
          if (spy) {
            DiscoveryMsg revMsg = receiveMsg(this);
            if (logger.finerOn()) {
          ...
          pdu = (DiscoveryPDU) receiveMsg(this);
  ...
  private void sendEvent(int state) {
    ...
    try {
      sendMsg(pdu, multicastGroup, multicastPort);
  ...
  private void processMsg(DiscoveryPDU rcvPDU)
    ...
    try {
      ...
      sendMsg(pdu, receiverInetAddr, receiverPort);
    } catch (IOException e) {
      if (logger.finerOn()) {
        ...
        try {
          ...
          for (Object o : result) {
            ...
            try {
              ...
              int port = ((Integer) cmf.getAttribute(objectName, "Port")).intValue();
