...
public final class ClassLoaderRepositorySupport
  ...
  private static class LoaderEntry {
    final ObjectName name; // can be null
   ...
   **/
  private synchronized void add(ObjectName name, ClassLoader cl) {
   ...
   **/
  private synchronized void remove(ObjectName name, ClassLoader cl) {
  ...
  private Class loadClass(final LoaderEntry list[],
    ...
    for (LoaderEntry loaderEntry : list) {
      try {
        final ClassLoader cl = loaderEntry.loader;
