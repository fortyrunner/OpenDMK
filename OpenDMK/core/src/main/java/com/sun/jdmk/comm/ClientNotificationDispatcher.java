...
class ClientNotificationDispatcher implements ClientNotificationHandler {
   ...
   * @throws InstanceNotFoundException The MBean name provided does not
  ...
  public void removeNotificationListener(ObjectName mbean,
    ...
    synchronized (listenerList) {
      for (Object value : listenerList.values()) {

        li = (ListenerInfo) value;
  ...
  public void getNotifications() {
    ...
    if (ret != null) {
      for (Object o : ret) {
        RemoteNotification ne = (RemoteNotification) o;
        synchronized (listenerList) {
  ...
  private class ForwardNotifs {
    ...
    private final List notifList = Collections.synchronizedList(new ArrayList());
    ...
    private class TaskThread extends Thread {
      ...
      public void run() {
        while (!toBeTerminated) {
          ...
          synchronized (listenerList) {
            for (RemoteNotification notif : notifs) {
              ListenerInfo li = (ListenerInfo) listenerList.get(notif.id);
              if (li == null) {
                ...
                try {
                  li.listener.handleNotification(notif.notif, li.handback);
                } catch (Exception e) {
                  // TODO remove the listener?
                  if (logger.finestOn()) {
  ...
  protected synchronized void internalSetMode(int mode) {
    ...
    if (mode == ClientNotificationHandler.PUSH_MODE) {
      ...
      if (mode == ClientNotificationHandler.PULL_MODE) {
        // use pull mode
  ...
  private ListenerInfo findListenerInfo(ObjectName mbean,
    ...
    for (Object o : listenerList.values()) {
      ListenerInfo lis = (ListenerInfo) o;
  ...
  private final ForwardNotifs forwardNotifs = new ForwardNotifs();
