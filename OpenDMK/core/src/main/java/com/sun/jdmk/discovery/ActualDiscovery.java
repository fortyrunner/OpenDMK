...
class ActualDiscovery extends DiscoveryCommon {
  ...
  private final int RETURN_PORT_MIN_VAL = 1024;
  private final int RETURN_PORT_MAX_VAL = RETURN_PORT_MIN_VAL * 10;
  private int ttl = 1;
  ...
  class ReceivedMsgObj implements Runnable {
    ...
    private final ActualDiscovery discovery;
      ...
      try {
        while (!stopRequested) {
          try {
            ...
            // Retrieve msg
            // ------------------------
            DiscoveryMsg revMsg = receiveMsg(socket);
            if (revMsg == null) {
              if (logger.finerOn()) {
