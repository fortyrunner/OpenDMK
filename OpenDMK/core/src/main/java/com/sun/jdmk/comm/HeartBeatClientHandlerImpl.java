    ...
    if (heartbeatConnector.getRemoteMBeanServer().isConnected()) {
      if (period > 0 && old_period <= 0) {
        ...
        if (period <= 0 && old_period > 0) {
          ...
          if (period > 0 && old_period > 0 && period != old_period) {
      ...
      while (!toBeTerminated && period > 0) {
        ...
        try {
          ...
          if (heartbeatClientSessionId == null) {
            ...
            if (heartbeatServerSessionId == null) {
...
// 		    }
  ...
  private static final String localClassName = "HeartBeatClientHandlerImpl";
   ...
   */
  private final Hashtable handbackTable = new Hashtable();

  /**
   * Heartbeat listeners list containing the filter objects.
   */
  private final Hashtable filterTable = new Hashtable();
